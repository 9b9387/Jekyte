...
import { shell, BrowserWindow, protocol, app, ipcMain } from "electron";
...
axios.default = axios;
dotenv.config({
  path: require$$1.resolve(process.cwd(), ".env")
});
const _GitHubService = class _GitHubService {
  constructor() {
    __publicField(this, "state", "");
    __publicField(this, "githubApi", axios.create({
  ...
  async handleOAuthCallback(code, state2) {
    var _a, _b, _c, _d, _e, _f, _g, _h;
    console.log("handleOAuthCallback: ", code, state2);
    if (state2 !== this.state) {
      console.error("State mismatch:", state2, this.state);
      return;
    }
    try {
      ...
      if (data.access_token) {
        ...
        (_a = BrowserWindow.getAllWindows()[0]) == null ? void 0 : _a.webContents.send(
          "oauth-success",
          data.access_token
        );
      } else {
        console.error("No access token in response:", data);
        (_b = BrowserWindow.getAllWindows()[0]) == null ? void 0 : _b.webContents.send(
          "oauth-error",
      ...
      if (axios.isAxiosError(error)) {
        const errorMessage = ((_d = (_c = error.response) == null ? void 0 : _c.data) == null ? void 0 : _d.error_description) || error.message || "Failed to complete OAuth process";
        console.error("Axios error during OAuth:", {
          message: errorMessage,
          response: (_e = error.response) == null ? void 0 : _e.data,
          status: (_f = error.response) == null ? void 0 : _f.status
        });
        (_g = BrowserWindow.getAllWindows()[0]) == null ? void 0 : _g.webContents.send(
          "oauth-error",
          errorMessage
        );
      } else {
        console.error("Unexpected error during OAuth:", error);
        (_h = BrowserWindow.getAllWindows()[0]) == null ? void 0 : _h.webContents.send(
          "oauth-error",
...
app.on("open-url", async (event, url2) => {
  ...
  if (url2.startsWith("jekyte://github-oauth/callback")) {
    const code = new URL(url2).searchParams.get("code");
    const state2 = new URL(url2).searchParams.get("state");
    console.log("code: " + code);
    if (code && state2) {
      await githubService.handleOAuthCallback(code, state2);
    }
  }
});
